...
1. 클라이언트 TIME_WAIT, 서버 LAST_ACK

   ```
   클라이언트: TIME_WAIT
   서버: LAST_ACK (ACK 대기 중)
   ```

   - 원인: 클라이언트의 마지막 ACK가 손실됨
   - 현상: 서버가 FIN을 재전송함
   - 해결: 클라이언트가 ACK를 재전송하면 정상 복구

2. 클라이언트 TIME_WAIT, 서버 ESTABLISHED

   ```
   클라이언트: TIME_WAIT
   서버: ESTABLISHED (여전히 연결 유지)
   ```

   - 원인
     - 서버가 클라이언트의 FIN을 받지 못함
     - 또는 서버 애플리케이션이 소켓을 닫지 않음
   - 문제점: 서버 리소스 낭비 (좀비 연결)
   - 해결: 서버 측에서 강제 연결 종료 필요

3. 클라이언트 TIME_WAIT, 서버 CLOSE_WAIT

   ```
   클라이언트: TIME_WAIT
   서버: CLOSE_WAIT (애플리케이션이 close() 호출 안함)
   ```

   - 원인: 서버 애플리케이션 레벨에서 소켓을 닫지 않음
   - 문제점: 가장 흔한 소켓 리소스 누수 원인
   - 해결: 애플리케이션에서 명시적으로 소켓 close() 호출
...
- 외부 API를 짧은 시간에 많이 호출할 때
- 매번 새로운 RestTemplate을 만들 때
...
for (int i = 0; i < 1000; i++) {
    RestTemplate restTemplate = new RestTemplate(); // 매번 새로 생성
...
- 외부 API 호출 후 연결을 제대로 닫지 않을 때
- try-catch만 쓰고 finally 없을 때
...
- 데이터베이스 연결을 수동으로 관리할 때
- @Transactional 없이 직접 Connection 사용할 때
...
public class UserRepository {
    ...
    public User findUser(Long id) {
        Connection conn = dataSource.getConnection(); // 가져오기만 함
        // 비즈니스 로직...
        return user;
        // conn.close() 없음!
    }
...
```bash
# 1. CLOSE_WAIT 상태 확인 (서버 문제)
netstat -an | grep CLOSE_WAIT | wc -l

# 2. TIME_WAIT 상태 확인 (클라이언트 문제)
...
1. 가장 많은 문제는 클라이언트 TIME_WAIT 고갈
   - RestTemplate을 매번 새로 만들지 말고 Bean으로 설정해서 재사용하세요.
2. 두 번째는 서버 CLOSE_WAIT 누적
   - 외부 API 호출 후 conn.disconnect() 빼먹지 마세요.
3. 간단한 확인 방법
   - netstat -an | grep CLOSE_WAIT | wc -l 명령으로 50개 이상이면 문제 있는 겁니다.
